...
description: Claude-specific specialty methodology for constructing compact (<150 lines), expressive Claude Code subagent prompts using lambda contracts and symbolic logic. Use when creating new specialized Claude subagents with agent composition, MCP tool integration, or skill references. Not a general Codex custom agent prompt construction guide. Validated with phase-planner-executor (V_instance=0.895).
version: 1.0
status: validated
v_meta: 0.709
v_instance: 0.895
...
**Scope**: This is a Claude Code subagent prompt specialty skill. For platform-neutral workflow prompt design, use BAIME prompt refinement patterns in `methodology-bootstrapping` before adapting the result to a host-specific agent format.
...
**Usage**: See templates/subagent-template.md for structure. Apply integration patterns for Claude Code subagent features. Validate compactness (≤150 lines), integration (≥1 feature), clarity. Reference examples/ for compact demonstrations and case-studies/ for detailed analysis.
