...
This skill provides the complete BAIME framework for systematic methodology development. The methodology is especially powerful when combined with AI coding assistants that can execute the OCA cycle, coordinate workflow agents, and calculate value functions automatically.
...
BAIME provides specialized workflow agents to streamline experiment execution. Invoke them through your host's agent mechanism. For Claude, this may be the Task tool with `subagent_type`; for Codex, use the matching custom agent from `.codex/agents/`.
...
**How to invoke**:
```
Invoke the `iteration-prompt-designer` workflow agent.

Claude example: use the Task tool with subagent_type="iteration-prompt-designer".
Codex example: use the `iteration-prompt-designer` custom agent.
...
**How to invoke**:
```
Invoke the `iteration-executor` workflow agent.

Claude example: use the Task tool with subagent_type="iteration-executor".
Codex example: use the `iteration-executor` custom agent.
...
- Transforms experiment artifacts into production-ready BAIME skills
...
**How to invoke**:
```
Invoke the `knowledge-extractor` workflow agent.

Claude example: use the Task tool with subagent_type="knowledge-extractor".
Codex example: use the `knowledge-extractor` custom agent.
...
- ✅ Reusable BAIME skills (ready for distribution)
...
Converge → Create results.md
       ↓
Extract → knowledge-extractor → shared skill source + knowledge/
       ↓
Distribute → Claude and Codex users through host entrypoints
```
...
2. Invoke the `iteration-executor` workflow agent to execute iteration with structured documentation
3. Review [examples/iteration-documentation-example.md](examples/iteration-documentation-example.md) for quality reference

**Automated generation**: Use the `iteration-executor` workflow agent to ensure consistent structure and systematic value calculation.
...
**Automated generation**: Use the `iteration-prompt-designer` workflow agent to create domain-specific iteration prompts.
...
**Automated execution**: Use the `iteration-executor` workflow agent to ensure consistent structure and systematic value calculation.
...
✅ **Do**:
- Use the `iteration-executor` workflow agent for consistent structure
...
**Terminology**: This skill implements the **Bootstrapped AI Methodology Engineering (BAIME)** framework. Use "BAIME" when referring to this methodology in documentation, research, or when asking an AI coding assistant for assistance with methodology development.
...
| File references | File content pasted inline | Host-supported file/line references (for Claude, `@file:line-range`) |
