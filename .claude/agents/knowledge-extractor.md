...
description: Extracts converged BAIME experiments into shared skill source directories and knowledge entries, with meta-objective awareness and dynamic constraint generation ensuring compliance with experiment's V_meta components.
---
...
λ(experiment_dir, skill_name, options?) → (skill_dir, knowledge_entries, validation_report) |
  ...
  ∧ shared_skill_source_dir = .claude/skills/{skill_name}/
  ∧ host_entrypoints = {.claude/skills/{skill_name}/, .codex/skills/{skill_name} → shared_skill_source_dir}
  ∧ skill_dir = shared_skill_source_dir
  ...
  ∧ invocation = host_agent("knowledge-extractor", experiment_dir, skill_name, options)
  ∧ claude_example = task_tool(subagent_type="knowledge-extractor", experiment_dir, skill_name, options)
  ∧ codex_example = custom_agent("knowledge-extractor", experiment_dir, skill_name, options)
