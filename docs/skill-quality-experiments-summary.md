...
|---|---|---|
| feature-to-backlog | **0.876** | 0.831 |
| backlog-setup | **0.878** | 0.849 |
| 参考：loop-backlog/task-from-template | 0.92 | — |
| 参考：task-to-backlog | 0.667 | — |

**跨 skill 方差**：σ(verdict_only) = **0.001**（远低于 0.10 阈值）

**假设判决**：**H-universal CONFIRMED**（σ=0.001 < 0.10）

### 各 Class 阈值验证

| Class | 全局阈值 | feature-to-backlog | backlog-setup | 状态 |
|---|---|---|---|---|
| A (binary-gate) | ≥ 0.85 | 0.900 | 0.900 | ✓ 满足 |
| B (invariant-check, verdict-only) | ≥ 0.70 | 0.795 | 0.813 | ✓ 满足 |
| C (branch-selection, verdict-only) | ≥ 0.80 | 0.953 | 0.947 | ✓ 满足 |

### 结论

跨 skill 方差极低（σ=0.001），全局阈值表对两个新 skill 均成立。**推荐：global-threshold**（无需 per-skill 标定）。Layer 2.5 oracle 框架已具备足够的跨 skill 泛化能力，可纳入统一发行门设计。
