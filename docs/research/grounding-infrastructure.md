...
**两个子类型（D1 / D2）**：现有描述把行为接地默认等同于"产品在真实环境的运行行为"，但这只是其中一类。行为接地实际有两个子类型，对不同性质的被测对象，ground truth 来源不同：

- **D1 — 生产遥测（production telemetry）**：部署后产品的运行指标（用户路径、错误率、性能基线、转化漏斗）。对**产品类**交付物，这是行为 ground truth。可自动化（webhook / 遥测管线），但属中长期建设。
- **D2 — 跨上下文可移植性（cross-context portability）**：方法论/工具在一个**新项目**里能否实际用起来。对 **BAIME 这种方法论框架**（而非产品），D2 才是行为 ground truth——本仓库内部的 escape rate 只是弱代理，强信号是"在别的项目里试用时发生了什么"。D2 现在就是最贵的决策点，且无工具支撑（手工试用、手工观察）。

**对 BAIME 的含义**：escape rate（D2 的仓内弱代理）回答"内部发现的问题"，但 BAIME 是否真的有效，由"换一个项目还能不能用"决定。这条 D2 通道是当前 grounding 缺口里最贵的一段——见跨项目验证 epic（cross-project validation / D2 portability）。
...
**外部基准**：arxiv 2503.12374（[Beyond Final Code: Process-Oriented Error Analysis of Software Development Agents](https://arxiv.org/pdf/2503.12374)）在真实 GitHub 场景中分析 agent 实际执行过程而非最终产出，是§9 进程接地方向最近的学术对应物。2026 年行业调查显示 33% AI 部署完全缺乏可验证的 audit trail，61% 基础设施碎片化到无法产出可操作证据——本节缺口分析反映的是行业普遍现状而非 BAIME 的特例。
...
**外部先例**：arxiv 2603.15676（[Automated Self-Testing as a Quality Gate](https://arxiv.org/html/2603.15676v1)）在 LLM 应用发布层面实现了结构上相似的 evidence-driven gate（PROMOTE/HOLD/ROLLBACK），并发现 evidence coverage 是最重要的严重回归判别器。与本节方案 1 的 gate evidence pack 设计意图一致，但作用粒度不同：该论文的 gate 作用于**应用发布**，本节方案 1 作用于**单次任务执行**。
...
1. **meta-cc + archguard 成为可移植工具**：当四阶段回路中每个钩子点都有明确的工具调用规范，这套接地基础设施就不再是"BAIME 项目的特定做法"，而是可以被任何 loop-backlog 实例化的工程模式。接地原则（§7）从设计原则变为操作规程。LangSmith / MLflow / Arize Phoenix 等商业 LLM 可观测性工具（2026 年市场规模 $2.69B，CAGR 36.2%）覆盖同类 trace 数据，但方向相反：商业工具把 trace 向外导出供运维人员监控，meta-cc 把同一数据**向内喂回**同一 session 的 gate 决策点——这个闭环用法是 BAIME 的差异化设计。
