...
preflight_agent_installer() {
    ...
    if run_logged "install-codex-agents.sh --scope user --dry-run" "$RESULT_DIR/agent-installer-dry-run.txt" "$installer" --scope user --dry-run; then
        pass "agent installer dry-run generated and validated 6 portable agents"
    else
    ...
    if run_logged "install-codex-agents.sh --scope project --target <temp>" "$RESULT_DIR/agent-installer-project.txt" "$installer" --scope project --target "$tmp_project"; then
        pass "agent installer wrote agents into a temporary project"
    else
    ...
    if assert_installed_agents_portable "$tmp_project/.codex/agents" >> "$PREFLIGHT_LOG" 2>&1; then
        pass "temporary project agents are self-contained and portable"
    else
        fail "temporary project agents are invalid or not portable"
    fi
    ...
    if assert_installed_agents_portable "$tmp_project/.codex/agents" >> "$PREFLIGHT_LOG" 2>&1; then
        pass "forced project agents remain self-contained and portable"
    else
        fail "forced project agents are invalid or not portable"
    fi
...
preflight_skills() {
    ...
    if [ "$count" = "19" ]; then
        pass ".codex/skills contains 19 symlinked skill entries"
    else
        fail ".codex/skills must contain 19 symlinked skill entries, found $count"
    fi
