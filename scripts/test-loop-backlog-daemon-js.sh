#!/bin/bash
# Tests for scripts/loop-backlog-daemon.js (Node.js version)
# Must be run from repo root.

set -e

REPO_ROOT=$(git rev-parse --show-toplevel)
DAEMON="$REPO_ROOT/scripts/loop-backlog-daemon.js"
TASKS_DIR=$(mktemp -d)
...
test_daemon_writes_pid_file() {
    rm -f "$PID_FILE"
    node "$DAEMON" --tasks-dir "$TASKS_DIR" --pid-file "$PID_FILE" \
...
echo "=== loop-backlog-daemon-js tests ==="
test_daemon_writes_pid_file
test_daemon_emits_task_ready_line
test_daemon_debounces_repeated_ready
test_daemon_re_emits_after_status_reset
