---
name: workflow-coach
description: Analyzes your Claude Code workflow patterns through structured coaching conversations, identifies bottlenecks, and delivers actionable improvement recommendations. Works standalone; can optionally use meta-cc session data for richer evidence.
---

# Workflow Coach

**Help users optimize their Claude Code workflows through structured coaching conversations, pattern analysis, and evidence-based recommendations.**
...
- How long have you been using Claude Code on this project?
...
Ask the user to walk through a recent Claude Code session from beginning to end:
...
- Did you use `@file` references, `/` commands, or `@agent-` subagents? How often?
...
- What frustrates you most about your current Claude Code workflow?
- Are there tasks you repeat frequently that feel manual or inefficient?
- Do you often find yourself re-explaining context that Claude should already have?
- How often do you need to correct Claude's output, and in what situations?
...
**Definition**: How completely and efficiently does the user provide context to Claude?

Indicators to look for:
- Frequent use of `@file` or `@dir` references → high context provision
...
- Use of specific line ranges (`@file.ts:20-45`) → advanced context provision

**Maturity levels**:
| Level | Description |
|-------|-------------|
| Novice | Mostly prose descriptions; rarely uses `@` references |
| Developing | Uses `@file` for major files; misses supporting context |
| Proficient | Consistently references relevant files; occasionally specifies lines |
| Expert | Uses precise `@file:line-range`; bundles all relevant context upfront |

### 2.2 Delegation and Tool Usage

**Definition**: Does the user choose the right tool (direct prompt, `/command`, `@agent-`) for each task type?

Indicators:
- Repeated identical workflows done manually → delegation opportunity missed
- Using a direct prompt for a task with an existing `/command` → underutilization
- Using subagents for complex multi-step tasks → good delegation
- Over-delegating simple queries to subagents → unnecessary overhead

**Maturity levels**:
| Level | Description |
|-------|-------------|
| Novice | Direct prompts for everything |
| Developing | Aware of `/commands` but uses them inconsistently |
| Proficient | Uses `/commands` for routine tasks; delegates complex work to subagents |
...
Indicators:
- Clarification requests from Claude → incomplete initial prompt
- User corrections immediately after Claude responds → misaligned expectations
...
|------------|-----------|-----------------|
| Underspecified context | Not knowing what Claude needs upfront | "I often have to paste the file content again" |
| Low delegation | Unfamiliarity with available commands or subagents | Manually repeating multi-step tasks |
| Verification skipping | Urgency bias; trusting Claude output without check | Discovering errors two steps later |
...
- "Start your next session by typing `@PLAN.md` before writing any prompt"
- "After Claude completes a task, run the test suite before writing your next prompt"
...
- "After each project milestone, conduct a 10-minute retrospective on your Claude Code habits"
...
- `user_focused`: analyze user-facing workflow decisions, not Claude's internal operations
